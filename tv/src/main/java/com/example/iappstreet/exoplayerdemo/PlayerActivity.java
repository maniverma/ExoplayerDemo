...
//    private VideoplazaPlugin mVideoPlazaPlugs;
...
//        mVideoPlazaPlugs = VideoplazaPlugin.getInstance();
    ...
    private void initializePlayer() {
        ...
        mPlaylist = (Playlist) getIntent().getBundleExtra(KEY_BUNDLE_EXTRA).getSerializable(
                KEY_PLAYLIST);
        if (mPlaylist == null)
            throw new IllegalStateException();
        else {
            mPlaylistItem = mPlaylist.getItems().get(mPlaylist.getCurrentItemIndex());
        }
       /* mCurrentPath = mPlaylistItem.getUrl();
        startPlay();*/

                 if (mPlaylistItem.hasAd()) {
                    fetchPreRollAndPlay();
                } else {
                     mCurrentPath = mPlaylistItem.getUrl();
                     startPlay();
                 }
                        // mPlaybackAsset = tv.accedo.wynk.android.airtel.tv.tvleanback.player.NextVideoPlayer.PLAYBACK_ASSET.CONTENT;
    ...
    private void fetchPreRollAndPlay() {
//        mVideoPlazaPlugs.setListener(new VideoplazaPlugin.AdUrlListener() {
//            @Override
//            public void getUrl(String mPrerollUrl) {
//                if (null != mPrerollUrl) {
//                    mCurrentPath = mPrerollUrl;
//                    checkStreaming();
//                } else
//                {
//                    mPlaybackAsset = PLAYBACK_ASSET.CONTENT;
//                    mCurrentPath = mPlaylistItem.getUrl();
//                }
//                startPlay();
//
//            }
//        });
//        mPlaybackAsset = PLAYBACK_ASSET.AD;
//        mAdType = AD_TYPE.PREROLL;
//        mVideoPlazaPlugs.onPlayBackComplete();
//        mVideoPlazaPlugs.init(this, Integer.parseInt(
//                mPlaylistItem.getMetadata().get(PlaylistItem.ASSET_VIDEO_DURATION)),
//                mPlaylistItem.getId(), mPlaylistItem.getMetadata().get(PlaylistItem.ASSET_TITLE));
    }
    ...
    private void checkStreaming() {
        if (!TextUtils.isEmpty(mCurrentPath)) {
            if (mCurrentPath.startsWith("http://")
                    || mCurrentPath.startsWith("https://")
                    || mCurrentPath.startsWith("rtsp://")
                    || mCurrentPath.startsWith("mms://")) {
                mIsStreaming = true;
            } else if (mCurrentPath.startsWith("file://")) {
                mIsStreaming = false;
                mCurrentPath = mCurrentPath.replace("file://", "");
            }
        }
    }
}
